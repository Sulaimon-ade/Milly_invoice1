        ...
        <div ref={ref} className="bg-white p-12 shadow-2xl max-w-4xl mx-auto">
              ...
              <div className="flex-1">
                ...
                <div className="text-gray-600 space-y-1">
                  ...
                  {businessInfo.instagram_handle && (
                    <p className="text-purple-600">{businessInfo.instagram_handle}</p>
                  )}
                </div>
              </div>
            ...
            <div className="w-80">
              ...
              {invoiceData.delivery_fee > 0 && (
                <div className="flex justify-between py-2 text-gray-700">
                  ...
                  <span className="font-semibold">
                    ₦{invoiceData.delivery_fee.toFixed(2)}
                  </span>
                </div>
              )}

              {invoiceData.refundable_caution_fee > 0 && (
                <div className="flex justify-between py-2 bg-yellow-50 px-3 rounded border-l-4 border-yellow-400">
                  <span className="font-bold text-yellow-900">
                    Refundable Caution Fee:
                  </span>
                  <span className="font-bold text-yellow-700">
                    ₦{invoiceData.refundable_caution_fee.toFixed(2)}
                  </span>
                </div>
              )}
          ...
          {(businessInfo.bank_name ||
            ...
            <div className="mb-6 md:mb-8 bg-gray-50 p-4 md:p-6 rounded-lg border-l-4 border-gray-400">
              <h3 className="text-xs md:text-sm font-semibold text-gray-900 uppercase mb-4">
                Bank Details
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-700 text-xs md:text-sm">

                {/* Account 1 */}
                <div>
                  <p className="text-gray-500 font-medium mb-2">Account 1</p>
                  {businessInfo.account_holder_name && (
                    <p><strong>Holder:</strong> {businessInfo.account_holder_name}</p>
                  )}
                  {businessInfo.bank_name && (
                    <p><strong>Bank:</strong> {businessInfo.bank_name}</p>
                  )}
                  {businessInfo.account_number && (
                    <p><strong>Number:</strong> {businessInfo.account_number}</p>
                  )}
                </div>

                {/* Account 2 */}
                {(businessInfo.account2_bank_name ||
                  businessInfo.account2_number ||
                  businessInfo.account2_holder_name) && (
                  <div>
                    <p className="text-gray-500 font-medium mb-2">Account 2</p>
                    {businessInfo.account2_holder_name && (
                      <p><strong>Holder:</strong> {businessInfo.account2_holder_name}</p>
                    )}
                    {businessInfo.account2_bank_name && (
                      <p><strong>Bank:</strong> {businessInfo.account2_bank_name}</p>
                    )}
                    {businessInfo.account2_number && (
                      <p><strong>Number:</strong> {businessInfo.account2_number}</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="border-t-2 border-gray-200 pt-4 md:pt-6 text-center">
            <p className="text-gray-700 mb-2 text-sm md:text-base">
              {businessInfo.thank_you_message}
            </p>
          ...
          <div className="bg-white p-12 shadow-2xl max-w-4xl mx-auto break-before-page">
            <div className="mb-12">
              <div className="flex items-center justify-center mb-8">
                ...
                <h1 className="text-4xl font-bold text-purple-700 mx-8">
                  Terms & Conditions
                </h1>
                <div className="flex-1 border-b-2 border-purple-600"></div>
              </div>
